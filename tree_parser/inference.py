...
from tree_parser.tree_v2 import Symbol, Node, Tree, Edge, ROOT_ID
from tree_parser.tree_latex import tree_to_latex
from tree_parser.score_tree import score_tree
...
from tree_parser.subset_selection import make_spatial_subsets, make_neighborhood_subsets, make_xaxis_subsets, _bbox_edge_dist
from tree_parser.tree_builder import (
    build_tree_from_evidence, build_tree_from_scores, find_seq_conflicts,
)
from tree_parser.evidence import jitter_bboxes
...
class TreeParser(ABC):
    ...
    def parse(self, symbols: list[DetectedSymbol],) -> tuple[str, float]:
        ...
        if not symbols:
            return "", 1.0
        if len(symbols) == 1:
            from latex_utils.glyphs import SYMBOL_TO_LATEX
    ...
    def parse_with_tree(
        ...
        if self.tree_strategy in ("backtrack", "backtrack_collapse"):
            from tree_parser.bottomup_v2 import build as build_v2, build_with_collapse
            v2_syms = self._make_symbols(names, bboxes)
