...
from engine.checkpoint import save_checkpoint, load_checkpoint, _checkpoint_path
from engine.stroke import Stroke
from engine.renderer import render_strokes
from classifier.stroke_augment import augment_strokes
...
class SymbolDataset(Dataset):
    ...
    def _render_from_json(self, json_path: Path) -> tuple[Image.Image, float]:
        with open(json_path) as f:
            data = json.load(f)
        strokes = [Stroke.from_dicts(pts) for pts in data["strokes"]]
