...
def measure_stats(items, augment=False):
    ...
    return {
        'sup_dy': sup_dy, 'sub_dy': sub_dy,
        'gap_by_et': dict(gap_by_et), 'h_ratio_by_et': dict(h_ratio_by_et),
        'num_off': num_off, 'den_off': den_off,
        'overlap_count': overlap_count, 'total_pairs': total_pairs,
    }
...
def main():
    ...
    # Print summary
    print(f'\n=== SUP/SUB dy ===')
    print(f'  SUP  HW: {np.mean(hw_s["sup_dy"]):.3f}±{np.std(hw_s["sup_dy"]):.3f}  Aug: {np.mean(aug_s["sup_dy"]):.3f}±{np.std(aug_s["sup_dy"]):.3f}  (extreme <-3: {sum(1 for d in aug_s["sup_dy"] if d<-3)})')
    print(f'  SUB  HW: {np.mean(hw_s["sub_dy"]):.3f}±{np.std(hw_s["sub_dy"]):.3f}  Aug: {np.mean(aug_s["sub_dy"]):.3f}±{np.std(aug_s["sub_dy"]):.3f}')
    ...
    for et in [-1, NUM, DEN, SUP, SUB]:
        ...
        if hg or ag:
            print(f'  {ET_NAMES[et]:5s}  HW: {np.mean(hg):.3f}±{np.std(hg):.3f}  Aug: {np.mean(ag):.3f}±{np.std(ag):.3f}  neg: {neg}/{len(ag)}')
    ...
    for et in [SUP, SUB, NUM, DEN]:
        ...
        if hr or ar:
            print(f'  {ET_NAMES[et]:5s}  HW: {np.mean(hr):.3f}±{np.std(hr):.3f}  Aug: {np.mean(ar):.3f}±{np.std(ar):.3f}')
    ...
    axes[0, 0].hist(hw_s['sup_dy'], bins=40, alpha=0.6, color='blue', density=True, label='HW', range=(-3, 1))
    axes[0, 0].hist(aug_s['sup_dy'], bins=40, alpha=0.6, color='red', density=True, label='Aug', range=(-3, 1))
    axes[0, 0].set_title('SUP dy/parent_h')
    axes[0, 0].legend()

    axes[0, 1].hist(hw_s['sub_dy'], bins=30, alpha=0.6, color='blue', density=True, label='HW', range=(-0.5, 2))
    axes[0, 1].hist(aug_s['sub_dy'], bins=30, alpha=0.6, color='red', density=True, label='Aug', range=(-0.5, 2))
    ...
    for idx, et in enumerate([NUM, DEN]):
        hg = hw_s['gap_by_et'].get(et, [])
        ag = aug_s['gap_by_et'].get(et, [])
        axes[0, 2].hist(hg, bins=30, alpha=0.4, color='blue', density=True, label=f'HW {ET_NAMES[et]}', range=(-0.5, 2))
        axes[0, 2].hist(ag, bins=30, alpha=0.4, color='red' if et == NUM else 'orange', density=True, label=f'Aug {ET_NAMES[et]}', range=(-0.5, 2))
